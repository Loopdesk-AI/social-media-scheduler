...
export class WebhookController {
  ...
  async verifyFacebookWebhook(req: Request, res: Response) {
    ...
    const verifyToken =
      process.env.FACEBOOK_WEBHOOK_VERIFY_TOKEN || "social_scheduler_verify";
  ...
  private async processTwitterTweetCreated(tweet: any) {
    ...
    logger.info("Twitter tweet created", {
      tweetId: id_str,
      userId: user.id_str,
    });
  ...
  private async updatePostStatus(
    ...
    try {
      ...
      const integration = await db.query.integrations.findFirst({
        where: and(
          eq(integrations.internalId, accountId),
          eq(integrations.providerIdentifier, platform),
      ...
      const post = await db.query.posts.findFirst({
        where: and(
          ...
          eq(posts.releaseId, platformPostId),
          isNull(posts.deletedAt),
        ),
      });

      if (!post) {
        logger.debug("Post not found for webhook", {
          platformPostId,
          platform,
        });
        return;
  ...
  private async markIntegrationForRefresh(accountId: string, platform: string) {
    try {
      // Find and update all matching integrations
      const matchingIntegrations = await db.query.integrations.findMany({
        where: and(
          eq(integrations.internalId, accountId),
          eq(integrations.providerIdentifier, platform),
          isNull(integrations.deletedAt),
