    ...
    async post(
      ...
      const mediaIds = await Promise.all(
        (firstPost.media || []).map(async (m) => {
          ...
          // Only construct full URL if it's a relative path (for uploaded files)
          if (!mediaUrl.startsWith('http://') && !mediaUrl.startsWith('https://')) {
            const baseUrl = process.env.STORAGE_LOCAL_URL || 'http://localhost:3001/uploads';
            mediaUrl = `${baseUrl}/${m.path}`;
          }
