  ...
  implements SocialProvider
{
  ...
  public override handleErrors(
    ...
    if (body.indexOf("API access blocked") > -1) {
      return {
        type: "refresh-token" as const,
        value:
          "Instagram API access blocked. Please re-authenticate your account and ensure it's a Business/Creator account.",
      };
    }
  ...
  async post(
    ...
    const mediaIds = await Promise.all(
      (firstPost.media || []).map(async (m) => {
        ...
        if (
          ...
          const baseUrl =
            process.env.STORAGE_LOCAL_URL || "http://localhost:3000";
        ...
        if (containerData.error) {
          ...
          throw new Error(
            `Instagram API error: ${containerData.error.message || JSON.stringify(containerData.error)}`,
          );
        }
        ...
        while (status === "IN_PROGRESS" && attempts < maxAttempts) {
          ...
          console.log(
            `⏳ Container status: ${status} (attempt ${attempts}/${maxAttempts})`,
          );
        }

        if (status !== "FINISHED") {
          throw new Error(
            `Media processing failed with status: ${status} after ${attempts} attempts`,
          );
        }
    ...
    if (mediaIds.length === 0) {
      ...
      throw new Error(
        "Instagram requires media to be attached to posts. Please provide a media URL.",
      );
    } else if (mediaIds.length === 1) {
      // Single media post
      console.log(
        `Publishing single media post with creation ID: ${mediaIds[0]}`,
      );
      ...
      if (carouselData.error) {
        throw new Error(
          `Instagram API error: ${carouselData.error.message || JSON.stringify(carouselData.error)}`,
        );
      }
  ...
  async analytics(
    ...
    try {
      ...
      if (data.error) {
        ...
        if (
          data.error.code === 200 &&
          data.error.message === "API access blocked."
        ) {
          console.warn(
            "Instagram API access is blocked. This could be due to:",
          );
          console.warn("1. Invalid or expired access token");
          console.warn(
            "2. Instagram account not set up as business/creator account",
          );
        ...
        if (
          data.error.code === 100 &&
          data.error.message === "Param since is an invalid datetime."
        ) {
          console.warn(
            "Instagram API date format error. Using alternative date format.",
          );
