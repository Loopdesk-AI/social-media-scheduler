...
import { StorageAuthDetails, StorageFile, SearchOptions } from '../../base/storage.interface';
import { Readable } from 'stream';
import { DropboxError } from './dropbox.errors';
import { isSupportedMediaType, formatBytes, isSupportedMediaFile } from './dropbox.utils';
...
export class DropboxProvider extends BaseStorageProvider {
  ...
  constructor() {
    ...
    try {
      ...
      console.warn('Dropbox provider not configured:', (error as Error).message);
  ...
  private initializeDropboxClient(): void {
    ...
    const redirectUri = process.env.DROPBOX_REDIRECT_URI || `${process.env.BACKEND_URL || 'http://localhost:3001'}/api/storage/callback/dropbox`;

    console.log('Dropbox Config:', { clientId: !!clientId, clientSecret: !!clientSecret, redirectUri });
  ...
  async generateAuthUrl(): Promise<{
    ...
    if (!this.isConfigured) {
      throw new DropboxError('Dropbox provider not configured', 'NOT_CONFIGURED');
    }
  ...
  async authenticate(params: {
    ...
    try {
      ...
      if (!accessToken) {
        throw new DropboxError('Failed to retrieve access token', 'TOKEN_ERROR');
      }
      ...
      const quota = usage.allocation && 'allocated' in usage.allocation ? {
        used: usage.used,
  ...
  async refreshToken(refreshToken: string): Promise<StorageAuthDetails> {
    ...
    if (!refreshToken) {
      throw new DropboxError('No refresh token provided', 'MISSING_REFRESH_TOKEN');
    }
  ...
  async listFiles(
    ...
    try {
        ...
        .map(entry => {
          if (entry['.tag'] === 'file') {
            ...
            return {
              id: entry.id || '',
              name: entry.name,
              mimeType: entry['content_hash'] ? 'application/octet-stream' : 'application/octet-stream',
              size: entry.size || 0,
      ...
      return {
        files,
        nextPageToken: listResponse.result.has_more ? listResponse.result.cursor : undefined
      };
    } catch (error: any) {
      throw new DropboxError(
  ...
  async downloadFile(
    ...
    try {
      ...
      const mimeType = entry['content_hash'] ? 'application/octet-stream' : 'application/octet-stream';
    ...
    try {
      ...
      try {
        ...
        if (existingLinks.result.links && existingLinks.result.links.length > 0) {
          ...
          return sharedLink.replace('www.dropbox.com', 'dl.dropboxusercontent.com').replace('?dl=0', '?dl=1');
        }
      } catch (error) {
      ...
      const createLinkResponse = await dropboxWithToken.sharingCreateSharedLinkWithSettings({
        path: fileId
  ...
  async searchFiles(
    ...
    try {
      ...
      const files: StorageFile[] = searchResponse.result.matches
        ...
        .filter((match: any) => (match.metadata as any).metadata['.tag'] === 'metadata')
        .map((match: any) => {
        ...
        .filter((file: StorageFile | null): file is StorageFile => file !== null);

      return {
        files,
        nextPageToken: searchResponse.result.has_more ? searchResponse.result.cursor : undefined
      };
    } catch (error: any) {
      throw new DropboxError(
