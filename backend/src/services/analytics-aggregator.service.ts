...
import { integrations, posts } from "../database/schema";
...
export class AnalyticsAggregatorService {
  ...
  async getAggregatedAnalytics(
    ...
    const integrationsList = await db.query.integrations.findMany({
      where: and(
        ...
        eq(integrations.type, "social"),
        isNull(integrations.deletedAt),
        eq(integrations.disabled, false),
      ),
    });
    ...
    const platformAnalytics = await Promise.all(
      integrationsList.map(async (integration) => {
        try {
          ...
          logger.error(
            `Failed to fetch analytics for ${integration.providerIdentifier}`,
            {
              error,
            },
          );
    ...
    for (const { platform, analytics } of platformAnalytics) {
      ...
      for (const metric of analytics) {
        if (
          metric.label.toLowerCase().includes("impression") ||
          metric.label.toLowerCase().includes("view")
        ) {
      ...
      const platformPosts = postsList.filter(
        (p) => p.integration.providerIdentifier === platform,
      ).length;
    ...
    const topPerformingPosts = postsList.slice(0, 10).map((post) => ({
      id: post.id,
      content:
  ...
  async getBestTimesToPost(userId: string): Promise<BestTimeToPost[]> {
    ...
    const timeSlots = new Map<
      string,
      { engagementRate: number; count: number }
    >();
    ...
    const growthData = await Promise.all(
      integrationsList.map(async (integration) => {
        try {
          ...
          // Calculate growth
          const data = followerMetric.data.map((point: any, index: number) => {
            const growth =
              index > 0
          ...
          logger.error(
            `Failed to fetch audience growth for ${integration.providerIdentifier}`,
            {
              error,
            },
          );
  ...
  async exportToCSV(
    ...
    for (const point of analytics.timeSeriesData) {
      csv += `${point.date},All Platforms,${point.posts},${point.impressions},${point.engagements},${
        point.impressions > 0
          ? ((point.engagements / point.impressions) * 100).toFixed(2)
          : 0
      }%\n`;
    }
    ...
    csv += "\nPlatform Summary\n";
    csv +=
      "Platform,Total Posts,Total Impressions,Total Engagements,Engagement Rate\n";
