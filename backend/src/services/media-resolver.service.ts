...
export class MediaResolverService {
     ...
     */
    async resolveMedia(media: any[], userId: string): Promise<{ resolvedMedia: any[], cleanup: () => Promise<void> }> {
        ...
        try {
            for (let i = 0; i < resolvedMedia.length; i++) {
                ...
                const storageMatch = item.path.match(/^\/storage\/([^\/]+)\/files\/([^\/]+)$/);

                if (storageMatch) {
                    const [, integrationId, fileId] = storageMatch;
                    console.log(`⬇️  Resolving storage file: ${fileId} from integration: ${integrationId}`);
                    ...
                    try {
                        ...
                        const publicUrl = await this.storageService.getPublicUrl(integration, fileId);
                        ...
                        console.warn(`⚠️  Failed to get public URL, falling back to download: ${error}`);
                    }

                    // **FALLBACK: Download for YouTube or if public URL fails**
                    console.log(`📥 Downloading file for local access...`);
                    const { path: tempPath, filename, mimeType } = await this.storageService.downloadFileToTemp(integration, fileId);
