  ...
  return <div className="flex gap-6">
      ...
      <div className="flex-1 space-y-4">
        ...
        <div className="relative">
          <button onClick={() => setIsPlatformDropdownOpen(!isPlatformDropdownOpen)} className="flex items-center justify-between w-full bg-[#1a1a1a] text-white px-4 py-3 rounded-lg hover:bg-[#222] transition-colors border border-gray-800/50">
            ...
            <ChevronDown size={18} className={`text-gray-400 transition-transform ${isPlatformDropdownOpen ? 'rotate-180' : ''}`} />
          </button>
        </div>
        ...
        <div className="bg-[#1a1a1a] rounded-lg border border-gray-800/50 p-4">
          ...
          <div className="grid grid-cols-7 gap-1">
            {days.map((day, index) => <button key={index} onClick={() => day.isCurrentMonth && setSelectedDate(day.fullDate)} className={`
                  ...
                  ${!day.isCurrentMonth ? 'text-gray-600' : 'text-white hover:bg-gray-800'}
                  ${isSelectedDate(day.fullDate) ? 'bg-white text-black font-medium' : ''}
                `}>
                {day.date}
              </button>)}
          </div>
          <div className="pt-4 mt-4 border-t border-gray-800/50">
            <div className="flex items-center justify-center gap-2 mb-3">
              ...
              <input 
                ...
                aria-label="Minutes"
              />
              <button onClick={() => setSelectedPeriod(selectedPeriod === 'AM' ? 'PM' : 'AM')} className="px-3 py-2 bg-[#0a0a0a] text-white rounded border border-gray-800/50 hover:bg-gray-900 transition-colors" aria-label="Toggle AM/PM">
                {selectedPeriod}
              </button>
            </div>
            <div className="text-sm text-center text-gray-500">{getCurrentTimezone()}</div>
          </div>
        </div>
